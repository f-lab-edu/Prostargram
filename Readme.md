...
[발표 영상](https://www.youtube.com/watch?v=O_C9r5cduVE&t=348s) [발표 자료](https://docs.google.com/presentation/d/1SL7JEb8aAzBXU16tI7uoVpilk27ZLOj70KdnUQ9USdE/edit?usp=sharing)

# 1️⃣ 피드 조회 API 성능 최적화 (피드 시스템 설계 및 구현)
## 요구사항
...
RDBMS는 기본적으로 쓰기 연산보다는 읽기 연산 최적화에 강점을 두고 설계되었습니다. 이러한 RDBMS에서 게시물 생성시 수 많은 INSERT요청을 처리하는 것은 불가능하다고 판단했습니다.

## 인플루언서가 게시물을 작성하면 발생하는 문제
저스틴 비버가 술 취해서 게시물 수십개를 연달아 올렸고 트위터 서버가 터질뻔한 사례 설명

호날두는 인스타그램에서 무려 6억 5천만명의 팔로워를 보유하고 있습니다. 

이런 호날두나 저스틴 비버같은 인플루언서가 게시물을 작성하면 어떻게 될까요? 

[이미지]

Redis에 수억개의 INSERT문은 Redis에 큰 부하를 야기할 수 있습니다.

물론 메시지큐를 통해 천천히 데이터를 삽입할 수도 있지만, 인플루언서의 게시물 작성이 다른 피드 구성에 영향을 끼치는 것을 피하고 싶었습니다.
...
데이터가 분산되어 있어 게시물 데이터를 수정하고 싶다면, Redis 전체를 스캔하여 해당 게시물의 데이터를 수정해야합니다.

[이미지]

반면 post_id만 저장되어 있을 경우 게시물 캐시에 저장된 단일 데이터만 수정하면 됩니다.

## 팔로우 취소, 회원 탈퇴, 게시물 삭제의 경우 어떻게 해야할까?
유저가 팔로우를 취소하더라도 기존 Redis를 탐색하여 데이터를 지우는 것은 O(N)의 시간복잡도가 걸립니다.

물론 유저당 200개의 게시물만 저장할 수 있게 하여 큰 부하는 아니지만 다른 방법을 선택했습니다.

하지만, 유저가 회원 탈퇴를 하거나 하는 경우에는 유저를 팔로우 하고 있던 모든 유저의 피드에서 게시물을 삭제해야합니다.

혹은 게시물을 삭제할 경우 Redis 전체를 탐색하여 해당 게시물을 모두 삭제해야합니다.
